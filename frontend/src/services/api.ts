...
class ApiService {
    ...
    async getClassificationData(): Promise<ClassificationData[]> {
        return this.request<ClassificationData[]>(
            "/overview/classification-data"
        );
    }
    ...
    async getAnalyticsTimeSeries(
        ...
        return this.request<TimeSeriesData[]>(
            `/analytics/time-series?metric=${encodeURIComponent(
                metric
    ...
    async getClassificationPerformance(): Promise<ClassificationPerformance[]> {
        return this.request<ClassificationPerformance[]>(
            "/classification/performance"
        );
    }

    async getConfidenceData(): Promise<ConfidenceData[]> {
        return this.request<ConfidenceData[]>(
            "/classification/confidence-data"
        );
    }
    ...
    async getViolationExamples(category: string): Promise<any> {
        return this.request(
            `/violations/examples?category=${encodeURIComponent(category)}`
        );
    }
    ...
    async getModelDetails(modelId: string): Promise<ModelDetails> {
        return this.request<ModelDetails>(
            `/models/${encodeURIComponent(modelId)}/details`
        );
    }
}

// Inference API types
export type InferenceFamily = "encoder" | "baseline" | "sft" | "fraud_fuse";

export type InferencePredictSingleRequest = {
    model_path: string;
    family: InferenceFamily; // pass explicitly if model_path doesn’t clearly indicate
    text: string;
    threshold?: number;
    batch_size?: number;
};

export type InferencePredictBatchRequest = {
    model_path: string;
    family: InferenceFamily; // pass explicitly if model_path doesn’t clearly indicate
    inputs: { text: string }[];
    threshold?: number;
...
export type InferencePredictResponse = {
    model: string;
    family: InferenceFamily;
    labels: string[];
    threshold: number;
    n: number;
    predictions: { probs: number[]; pred: number[] }[];
};

// POST /api/inference/predict (supports single or batch)
export async function predictInference(
    payload: InferencePredictSingleRequest | InferencePredictBatchRequest,
    ...
    return res.json() as Promise<InferencePredictResponse>;
}
