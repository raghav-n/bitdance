...
@task(
    name="ingest_all",
    inputs=lambda params: ["configs/base.yaml", params["ingest"]["sources"][0]["path"]],
    outputs=[
...
def ingest_all(params: Dict[str, Any]):
    ...
    # Write outputs
    _write_outputs(
        df_text_only, df_with_images, image_arrays_deduplicated, logger, dataset_slug
    )
...
def _load_images_and_create_schema(
    ...
    image_base_path = Path(
        params.get("ingest", {})
        .get("image", {})
        .get("base_path", "data/archive/dataset")
    )
    ...
    df_unified = pd.DataFrame(
        {
            ...
            "has_image": df.apply(
                lambda row: create_review_id(row) in image_arrays, axis=1
            ),
    ...
    logger.info(
        f"Created unified schema with {len(df_unified)} records, {len(image_arrays)} with images"
    )
    return df_unified, image_arrays


def _deduplicate_reviews_with_images(
    df: pd.DataFrame, image_arrays: Dict[str, np.ndarray], logger
) -> Tuple[pd.DataFrame, Dict[str, np.ndarray]]:
    ...
    df_deduplicated = df.drop_duplicates(
        subset=["place_name", "user_name", "text_hash"], keep="first"
    )
...
def _write_outputs(
    ...
    summary = {
        ...
        "image_coverage": len(df_with_images) / len(df_text_only)
